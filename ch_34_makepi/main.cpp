...
// 푸아송 방정식으로 f(x) = x라하고 주기를 [-Phi, Phi]한 다음
// 풀면 파이는  Phi/4 = 1 -1/3 + 1/5 - 1/7 ... 입니다.
// 다음은 아주아주 거칠게 푼 파이 입니다.

void main()
{
	double Phi=0.;
	
	for(LONGLONG i=0; i<200000000; i+=4)
	{
		Phi += 1/double(i+1);
		Phi -= 1/double(i+3);
	}
...
// 아래소스는 베이리라는 사람이 1996년 빠른 16진수를 이용한 빠른 파일 계산법을 고안해
// 냈습니다.
// 그것을 가지고 만들었다군요.
